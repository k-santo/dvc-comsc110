...
bool isDupGuess(int* dupGuesses, int nDupGuesses1, int userGuess1); //check if user guess is a duplicate
...
int main()
{
  ...
  int nDupGuesses=0; //number of duplicate guesses
  int dupGuesses[MAX_SIZE]; //duplicate guess list
  ...
  while(true)
  {
    ...
    {  
      checkDupGuess= isDupGuess(dupGuesses, nDupGuesses, userGuess); //check for duplicate guess
      if(checkDupGuess==true) cout << "You already guessed " << userGuess << " -- guess again: ";
      
      //score loop, add guess to list
      if (checkDupGuess==false)
      {
        if (nDupGuesses < MAX_SIZE) dupGuesses[nDupGuesses++] = userGuess; //add guess to list
        if (userGuess < target)
        {
...
bool isDupGuess(int* dupGuesses, int nDupGuesses1, int userGuess1)
{
  //data
